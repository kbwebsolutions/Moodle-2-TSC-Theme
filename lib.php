...
function moodle2_tsc_require_course_login($courseorid, $autologinguest = true, $cm = NULL, $setwantsurltome = true, $preventredirect = true) {
    global $CFG, $SITE;
    ...
    if (!empty($CFG->forcelogin)) {
        ...
        moodle2_tsc_require_login($courseorid, $autologinguest, $cm, $setwantsurltome, $preventredirect);
...
function moodle2_tsc_require_login($courseorid = NULL, $autologinguest = true, $cm = NULL, $setwantsurltome = true, $preventredirect = false) {
    ...
    if ($course->id == SITEID) {
        ...
        // very simple enrolment caching - changes in course setting are not reflected imextmediately
        if (!isset($USER->enrol)) {
            $USER->enrol = array();
        ...
        if (is_viewing($coursecontext, $USER)) {
            ...
            if (isset($USER->enrol['tempguest'][$course->id])) {
                if ($USER->enrol['tempguest'][$course->id] == 0) {
                    ...
                    $USER->access = remove_temp_course_roles($coursecontext, $USER->access);
                }
            }
